...
| Adaptive replacement           | Implemented / QA-candidate | Immutable feedback, narrow `TOO_EXPENSIVE` inference, copy-on-write versioning, targeted replacement, and no-replacement handling.                   |
...
- Feedback, job creation, initial event, planning revision, and replay result are transactional.
